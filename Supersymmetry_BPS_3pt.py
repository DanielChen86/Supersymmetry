...
for i in range(1, 3):
    for I in range(2):
        for J in range(2):
            QA1[2] += (-1)*sig[i]*Num(ref*RA[i][:, I]) * \
                epsilon_L[I, J]*uDiff(str(i)+str(J)+'1')
...
for i in range(1, 3):
    for I in range(2):
        for J in range(2):
            QA2[1] += sig[i]*Num(ref*RA[i][:, I]) * \
                epsilon_L[I, J]*uDiff(str(i)+str(J)+'2')
...
solution = {}
solution[1] = etaP[1]*delta[1]-delta[1]*Ha[1]-delta[1] * \
    Hb[2]*etaP[1]*etaP[2]+delta[1]*Ha[1]*Hb[2]*etaP[2]
solution[2] = etaP[2]*delta[2]-delta[2]*Ha[2]+delta[2] * \
    Hb[1]*etaP[1]*etaP[2]+delta[2]*Ha[2]*Hb[1]*etaP[1]
