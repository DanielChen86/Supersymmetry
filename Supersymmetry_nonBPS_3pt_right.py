...
data = pd.read_csv(
    'spinor helicity data (3pt) (right)/spinor helicity data 1 (right).csv')
data = data['data']
...
LB[1] = np.matrix([[data[24]+data[25]*1.j, data[26]+data[27]*1.j],
                  [data[28]+data[29]*1.j, data[30]+data[31]*1.j]])
LB[2] = np.matrix([[data[32]+data[33]*1.j, data[34]+data[35]*1.j],
                  [data[36]+data[37]*1.j, data[38]+data[39]*1.j]])
...
def etas(particle, Rcharge):
    ...
    for I in range(2):
        for J in range(2):
            result += (-0.5)*epsilon_U[I, J]*SUSY(1., [str(particle) +
                                                       str(I)+str(Rcharge), str(particle)+str(J)+str(Rcharge)])
    return result
...
for A in range(1, 3):
    ...
    for I in range(2):
        for J in range(2):
            Qa[A] += (LA[1]*RA[2])[I, J]*uSUSY(str(1)+str(I) +
                                               str(A))*uSUSY(str(2)+str(J)+str(A))
...
for A in range(1, 3):
    ...
    for I in range(2):
        for J in range(2):
            Qb[A] += (LB[1]*RB[2])[I, J]*uSUSY(str(1)+str(I) +
                                               str(A))*uSUSY(str(2)+str(J)+str(A))
...
for A in range(1, 3):
    R[A] = SUSY_zeros()
    for I in range(2):
        R[A] += SUSY(Num(LB[0]*RB[1][:, I]), [str(1)+str(I)+str(A)])*etas(2, A) + \
            SUSY(Num(LB[0]*RB[2][:, I]), [str(2)+str(I)+str(A)])*etas(1, A)
