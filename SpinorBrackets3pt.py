...
data=pd.read_csv('spinor helicity data (3pt)/spinor helicity data 9.csv')
data=data['data']
...
for i in range(3):
    ...
    for j in range(4):
        Pvec[i][j,0]=np.trace(pL[i]*sigma[j]/2)
...
# print (abs(RB[0][0,0]-((pU[1]*RA[0])[0,0]*x12/m)),"should be close to 0")
