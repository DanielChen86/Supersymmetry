...
for A in range(1,3):
    ...
    for i in range(1,3):
        for I in range(2):
            deltaf[A]+=SUSY(Num(LB[0]*RB[i][:,I]),[str(i)+str(I)+str(A)+'f'])
...
for A in range(1,3):
    ...
    for i in range(1,3):
        for I in range(2):
            Haf[A]+=SUSY(Num(zeta*RB[i][:,I])/Num(zeta*RB[0]),[str(i)+str(I)+str(A)+'f'])
...
for A in range(1,3):
    ...
    for B in range(1,3):
        for i in range(1,3):
            for I in range(2):
                Hbf[A]-=SUSY(Num(xi*RA[i][:,I])*sig[i]*epsilonL(A,B)/Num(xi*RA[0]),[str(i)+str(I)+str(B)+'f'])
...
for i in (1,2):
    ...
    for (A,B) in product(*([range(1,3)]*2)):
        for (I,J) in product(*([range(2)]*2)):
            all_eta+=epsilon_U[I,J]*epsilonL(A,B)*uSUSYF(i,I,A,'f')*uSUSYF(i,J,B)
    exponent*=EXPft(all_eta)
del (all_eta)

def FT(amplitude):

    etaList=[[1,['01', '02', '101', '111', '102', '112', '201', '211', '202', '212']]]
